...
def download_data(asset_name, ticker_symbol, output_dir, start_date, end_date, max_retries=3):
...
def main():
    ...
    output_dir = "raw_data/external_data"
    os.makedirs(output_dir, exist_ok=True)

    # Download and save data for all assets
    for name, sym in assets.items():
        download_data(name, sym, output_dir, start_date, end_date)

    print("🎉 All available data processing complete!")

    df = pd.read_csv('raw_data/external_data/S&P_500_historical_data.csv')
    print(df)
