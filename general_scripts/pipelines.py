...
# PRICE MODEL
def price_model(data, scaler=StandardScaler(), model_type=LSTMModelMultiStep, criterion=nn.HuberLoss(), train_seq_len=60, test_seq_len=5, 
                tuning=True, epochs=50): 
    ...
    # data = data.drop(columns=['trend', 'seasonal'])
    if isinstance(data, pd.Series):
        data = data.to_frame()    
    train_size = len(data) - train_seq_len - test_seq_len
    ...
    y_pred = inverse_scale_predictions(y_pred_tensor, scaler).detach().cpu().numpy().ravel() + seasonal_next + trend_next
    y_true = inverse_scale_predictions(y_test_tensor, scaler).detach().cpu().numpy().ravel() + seasonal_last + trend_last

    print(y_pred)
    print(y_true)
    print(y_dates_test.shape)
    print(len(y_dates_test), len(y_true), len(y_pred))
